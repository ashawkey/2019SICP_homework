...
; (try-again i)
; understand at which point should we call fail!
...
(define (my-driver-loop)
  (define (inner-loop try-again)
    (let ((input (read))) 
      (if (eq? input eof)
          (void)
          (if (and (pair? input) (eq? (car input) 'try-again))
              (try-again (cadr input))
