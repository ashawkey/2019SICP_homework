...
(define (macro? exp) (tagged-list? exp 'define-syntax-rule))
...
(define (eval exp env)
  (cond ((self-evaluating? exp ) exp)
        ...
        ((application? exp)
           (if (and (exist-variable (operator exp) env) (or (primitive-procedure? (eval (operator exp) env))
                                                            (= (length (operands exp)) (length (mymlist->list (procedure-parameters (eval (operator exp) env)))))))
               (my-apply (eval (operator exp) env) (list-of-values (operands exp) env))
               (macro-apply (eval (make-macro-name (operator exp) (length (operands exp))) env) (operands exp)))
         )
        (else
...
(define (macro-apply procedure arguments)
  ...
  (define (replace var val exp)
    (cond ((null? exp) '())
          ((list? (car exp)) (cons (replace var val (car exp)) (replace var val (cdr exp))))
          ((eq? (car exp) var) (cons val (replace var val (cdr exp))))
          (else (cons (car exp) (replace var val (cdr exp))))))
  (define (replace-all vars vals exp)
    (if (null? vars) exp
        (replace-all (cdr vars) (cdr vals) (replace (car vars) (car vals) exp))))
  (eval-sequence
   (replace-all (mymlist->list (procedure-parameters procedure)) arguments (procedure-body procedure))
...
(define (macro-variable exp)
  (make-macro-name (caadr exp) (length (cdadr exp))))

(define (macro-value exp) 
  (make-lambda (cdadr exp)             
               (cddr exp)))

(define (eval-macro exp env)
  (define-variable! (macro-variable exp)
    (eval (macro-value exp) env) env)  
  (void))
