...
; while switch for break 
; key points: (eval-xxx), eval-sequence will return 'break if there is a break.
...
(define (eval exp env)
  (cond ((self-evaluating? exp ) exp)
        ...
        ((switch? exp) (eval-switch (cdr exp) env))
        ((for? exp) (eval-for (cdr exp) env))
        ((break? exp) 'break)
        ((lambda? exp)
...
(define (eval-while exp env)
  (let ([predicate (car exp)]
    ...
    (if (true? (eval predicate env))
        (begin (eval-sequence seq env) (eval-while exp env))
        (void))))

(define (switch? exp) (tagged-list? exp 'switch))
(define (eval-switch exp env)
  (let ([x (eval (car exp) env)])
    (let loop ([cases (cdr exp)])
      (let* ([case (car cases)]
             [c (car case)]
             [seq (cdr case)])
        (if (or (eq? c 'default) (equal? x (eval c env)))
            (eval-sequence seq env)
            (loop (cdr cases)))))))
...
(define (eval-for exp env)
  (let ([init (car exp)]
    ...
    (let loop ()
      (if (true? (eval predicate env))
          (if (eq? (eval-sequence seq env) 'break)
              (void)
              (begin (eval step env) (loop)))
          (void)))))
...
(define (eval-sequence exps env)
  (cond ((last-exp? exps) (eval (first-exp exps) env))
        (else (if (eq? (eval (first-exp exps) env) 'break)
                  'break
