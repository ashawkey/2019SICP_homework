    ...
    (let ((the-ops (list (list 'initialize-stack (lambda () (stack 'initialize)))))
      ...
      (define (dispatch message)
        (cond ((eq? message 'start)
              ...
              ((eq? message 'traceback) traceback)
...
; basic procedures
(define (make-execution-procedure inst labels machine pc flag stack traceback ops)
  (cond ((eq? (car inst) 'assign)
        ...
        ((eq? (car inst) 'call)
         (make-call inst machine labels traceback pc))
        ((eq? (car inst) 'ret)
         (make-ret inst machine labels traceback pc))
        (else (error "Unknown instruction type -- ASSEMBLE" inst))))

; swap = (swap (reg a) (reg b))
(define (make-swap inst machine pc)
  (let ((reg-a (get-register machine (cadadr inst)))
        (reg-b (get-register machine (car (cdaddr inst)))))
    (lambda ()
      (let ((tmp-a (get-contents reg-a)))
        (set-contents! reg-a (get-contents reg-b))
        (set-contents! reg-b tmp-a)
        (advance-pc pc)))))

; call = (call (label xxx))
(define (make-call inst machine labels traceback pc)
  (let ([dest (cadr inst)])
    (lambda ()
      (let ((insts (lookup-label labels (label-exp-label dest))))
        (push traceback (get-contents pc))
        (set-contents! pc insts)))))

; ret = (ret)
(define (make-ret inst machine labels traceback pc)
  (lambda ()
    (if (empty? traceback)
        (set-contents! pc '())
        (begin (set-contents! pc (pop traceback))
               (advance-pc pc)))))
