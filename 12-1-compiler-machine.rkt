...
; instruction = (registers-needs registers-maybe-modified assembly-codes)
...
(define (list-union s1 s2) ;merge s1 & s2 and remove the repeated elements
  (cond ((null? s1) s2)
        ((memq (car s1) s2) (list-union (cdr s1) s2))
        (else (cons (car s1) (list-union (cdr s1) s2)))))

(define (list-difference s1 s2) ;remove elements which are in s2 from s1
  (cond ((null? s1) '())
...
(define (end-with-linkage linkage instruction-sequence) ;返回一个指令序列
  (preserving '(continue) ;需要关注coutinue是否可能被instruction-sequence修改且在第二个指令序列中用到 
...
(define (compile-definition exp target linkage)
  (let ((var (definition-variable exp))
        (get-value-code (compile (definition-value exp) 'val 'next))) ; 变量值求出后放入val,接下来用val值对变量名赋值,因此linkage是 'next
    (end-with-linkage linkage (preserving '(env) ;env可能在get-value-code中被修改。为何没 val?
                                          get-value-code
                                          (make-instruction-sequence '(env val)
                                                                     (list target)
                                                                     `((perform (op define-variable!) (const ,var);变量名
                                                                                (reg val) ;变量值
                                                                                (reg env)) ;变量所在环境
...
(define (compile-assignment exp target linkage)
  (let ((var (assignment-variable exp))
    ...
    (end-with-linkage linkage
                      (preserving '(env)
                                  get-value-code
                                  (make-instruction-sequence '(env val) (list target) `((perform (op set-variable-value!)
                                                                                                 (const ,var) ;变量名
                                                                                                 (reg val) ;变量值
                                                                                                 (reg env)) ;变量所在环境
                                                                                        (assign ,target (const ok))))))))
  ...
  (let ((t-branch (make-label 'true-branch)) ;make-label生成无重复标号
    ...
    (let ((consequent-linkage (if (eq? linkage 'next) after-if linkage)))
      (let ((p-code (compile (if-predicate exp) 'val 'next))
        ...
        (preserving '(env continue) p-code (append-instruction-sequences
...
(define (compile-lambda-body exp proc-entry) ;proc-entry是个标号
  (let ((formals (lambda-parameters exp)))
    (append-instruction-sequences
     (make-instruction-sequence '(env proc argl)
                                '(env) ;函数体的代码要求执行前env放着环境,proc放着译后过程,argl放着实参值表
                                `(,proc-entry (assign env (op compiled-procedure-env) (reg proc))
                                              (assign env (op extend-environment) (const ,formals) (reg argl) (reg env))))
     (compile-sequence (lambda-body exp) 'val 'return)))) ;整个程序中唯一使用 'return 作为linkage参数的地方 ;函数体执行结束一定是 (goto (reg continue))

(define (compile-application exp target linkage)
  (let ((proc-code (compile (operator exp) 'proc 'next)) ;整个程序中,唯一 target不是 val 的地方。proc用来放“译后过程”
        (operand-codes (map (lambda (operand) (compile operand 'val 'next)) (operands exp))))
    (preserving '(env continue);求值运算符会改变env和continue
                proc-code (preserving '(proc continue) (construct-arglist operand-codes) (compile-procedure-call target linkage)))))
; construct-arglist 生成构造实参表的代码 compile-procedure-call 生成调用过程的代码组合式的编译
...
(define (compile-procedure-call target linkage) ;仅被compile-application调用
  ...
  (cond ((and (eq? target 'val)
         ...
         (make-instruction-sequence '(proc)
                                    ...
                                    `((assign continue (label ,linkage))
                                      ...
                                      (goto (reg val))))) ((and (not (eq? target 'val)) ;此时target是 'proc
                                                                (not (eq? linkage 'return)))
                                                           (let ((proc-return (make-label 'proc-return)))
                                                             (make-instruction-sequence '(proc)
                                                                                        ...
                                                                                        `((assign continue (label ,proc-return))
                                                                                          (assign val (op compiled-procedure-entry) (reg proc))
                                                                                          (goto (reg val)) ,proc-return (assign ,target (reg val))
                                                                                          (goto (label ,linkage))))))
                                                          ((and (eq? target 'val)
                                                                (eq? linkage 'return))
