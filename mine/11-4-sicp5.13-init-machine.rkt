; http://lisp.test.openjudge.org/extinterpreter/3/
...
(define (run-machine machine input output) 
  ...
  (if (= assemble-flag 0) (void)
      (begin (init-machine machine input)
             (start machine)
      ...
      (let ((m-txt (read)))
            (if (eq? m-txt eof)
                ...
                (if (eq? (car m-txt) 'make-machine)
                    (begin (display "a new machine") (newline)
