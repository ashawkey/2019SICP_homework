; http://lisp.test.openjudge.org/2019hw10/2/
#lang racket
...
(define (eval exp env)
  ;(display "eval ")(displayln exp)
  (cond ((self-evaluating? exp) exp)
        ((null? exp) (void)) 
        ((variable? exp) (lookup-variable-value exp env))
        ((quoted? exp) (eval-quoted (cadr exp) env))
        ((assignment? exp) (eval-assignment exp env))
        ((definition? exp) (eval-definition exp env))
        ((if? exp) (eval-if exp env))
        ((and? exp) (eval-and (cdr exp) env))
        ((or? exp) (eval-or (cdr exp) env))
        ((cons? exp) (eval-cons (cdr exp) env))
        ((car? exp) (eval-car (cdr exp) env))
        ((cdr? exp) (eval-cdr (cdr exp) env))
        ((lambda? exp)
...
; how to make a quote: (list 'quote exp)
(define (make-quote a)
  (list 'quote a))

; how to make an application
(define (make-application a b)
  (cons a b))

; turn list to lazy cons 
(define (eval-quoted exp env)
  (define (helper exp)
    (if (null? exp) '()
        (make-cons (make-quote (car exp)) (helper (cdr exp)))))
  (if (pair? exp)
      (eval (helper exp) env)
      exp))
...
; lazy cons, exp should also be lazy, so use lambda returning lambda.
; must use (list) for make-lambda params.
(define (eval-cons exp env)
  (eval (make-application (make-lambda (list 'a 'b)
                                       (list (make-lambda (list 'f) (list (list 'f 'a 'b)))))
                          exp)
        env))
...
(define (eval-car exp env)
  (eval (make-application (car exp) (list (make-lambda (list 'p 'q) (list 'p)))) env))

(define (cdr? exp)
  (tagged-list? exp 'cdr))

(define (eval-cdr exp env)
  (eval (make-application (car exp) (list (make-lambda (list 'p 'q) (list 'q)))) env))
        ...
        ((compound-procedure? procedure)
         ...
         (eval-sequence
          ...
          (extend-environment  
           ...
           (list->mlist (list-of-delayed-args arguments env))
...
(define (extend-environment vars vals base-env)
  (if (= (mlength vars) (mlength vals))
      (begin (mcons (make-frame vars vals) base-env))
      (if (< (mlength vars) (mlength vals))
