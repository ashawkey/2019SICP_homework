; http://lisp.test.openjudge.org/2019hw4/5/
...
(define (raise-rational n)
    (make-real (/ (+ ((get 'rational 'numer) n) 0.0) ((get 'rational 'denom) n))))
  (define (raise-real n)
...
(define (order s)
  (cond ((equal? (type-tag s) 'integer) 0)
        ((equal? (type-tag s) 'rational) 1)
        ((equal? (type-tag s) 'real) 2)
        ((equal? (type-tag s) 'complex) 3)))

(define (apply-generic op . args)
  (if (null? (cdr args))
      ; complex generic
      ((get op (list (caar args))) (cdar args))
      ; arithmetic generic
      (let ((x (car args))
            (y (cadr args)))
        (cond ((= (order x) (order y)) ((get op (list (type-tag x) (type-tag y))) (contents x) (contents y)))
              ((> (order x) (order y)) (apply-generic op x ((get 'raise (type-tag y)) y)))
              ((< (order x) (order y)) (apply-generic op ((get 'raise (type-tag x)) x) y))))))
  
(define (display-obj obj)
  (cond ((equal? (type-tag obj) 'integer) (displayln (list (car obj) (cdr obj))))
        ((equal? (type-tag obj) 'rational) (displayln (list (car obj) (cadr obj) (cddr obj))))
        ((equal? (type-tag obj) 'real) (displayln (list (car obj) (cdr obj))))
        ((equal? (type-tag obj) 'complex) (displayln (list (car obj) (real-part (cdr obj)) (imag-part (cdr obj)))))))
