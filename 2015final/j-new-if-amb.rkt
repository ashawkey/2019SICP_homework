...
(define (analyze exp)   
   (cond ((self-evaluating? exp) 
         ...
         ;;; mod
         ((new-if? exp) (analyze-new-if (cdr exp)))
...
;;; mod
(define (new-if? exp)
  (tagged-list? exp 'new-if))

(define (analyze-new-if exp)
  (lambda (env succeed fail)
    (let ([pred (analyze (car exp))]
          [exp1 (analyze (cadr exp))]
          [exp2 (analyze (caddr exp))])
      (pred env
            (lambda (val fail2) (exp1 env succeed fail2))
            (lambda () (exp2 env succeed fail))))))
