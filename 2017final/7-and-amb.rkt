...
(define (mydisplayln x) (displayln x) #t)
...
(define (analyze exp)
  ;(display " analyze: ") (displayln exp)
   (cond ((self-evaluating? exp) 
         ...
         ((and? exp) (analyze (and->if (cdr exp))))
...
(define (analyze-and exp)
  (let ((exp (map analyze exp)))
    (lambda (env succeed fail)
      (define (helper choices)
        (if (null? choices)
            (succeed #t fail) ; call the original succeed to get final value
            ((car choices) env
                           (lambda (val fail2)
                             (if (true? val)
                                 (helper (cdr choices))
