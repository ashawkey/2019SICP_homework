...
(define (delay-it mem exp env)
  (if mem
      (mlist 'memo-thunk exp env)
      (mlist 'thunk exp env)))
  ...
  (cond ((primitive-procedure? procedure)
        ...
        ((compound-procedure? procedure)
         (eval-sequence
          ...
          (extend-environment
           ;;; mod
           (procedure-parameters2 procedure)
           (list->mlist (list-of-args (procedure-parameters procedure) arguments env)) 
...
(define (list-of-args params exps env)
  (if (no-operands? exps) '()
      (let ([x (car exps)]
            ...
            [xp (mcar params)]
            [restp (mcdr params)])
        (cons (cond ((not (pair? xp)) (actual-value x env))
                    ((eq? (cadr xp) 'lazy) (delay-it false x env))
                    ((eq? (cadr xp) 'lazy-memo) (delay-it true x env)))
              (list-of-args restp rest env)))))
...
;;; mod
(define (procedure-parameters2 p)
  (list->mlist (map (lambda (x) (if (pair? x) (car x) x)) (cadr p))))
