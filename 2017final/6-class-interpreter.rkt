...
(define (eval exp env)
  (cond ((self-evaluating? exp ) exp)
        ...
        ;;; mod
        ((class? exp) (eval-class (cdr exp) env))
        ((new? exp) (eval-new (cdr exp) env))
        ((lambda? exp)
        ...
        ((application? exp)
         ...
         (let ((procedure (eval (operator exp) env)))
           (if (and (pair? procedure) (eq? (car procedure) 'class-env))
               (eval (cadr exp) (cadr procedure))
...
(define (eval-class exp env)
  (eval (cons 'define (list (car exp) (list 'quote (cdr exp)))) env))

(define (new? exp)
  (tagged-list? exp 'new))

;;; consider each class object as a new environment!
(define (eval-new exp env)
  (let ([seq (eval (car exp) env)]
        [new-env (extend-environment '() '() env)])
    (eval-sequence seq new-env)
    (list 'class-env new-env)))
...
(define (eval-assignment exp env)
  ;;; mod
  (if (pair? (cadr exp))
      (let ([class-env (cadr (eval (caadr exp) env))]
            [var (cadadr exp)]
            [val (caddr exp)])
        (set-variable-value! var (eval val env) class-env))
