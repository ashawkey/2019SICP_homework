...
(define eceval-operations 
		...
		(list 'user-print user-print)
                ...
                (list 'and? and?)
                (list 'or? or?)
                (list 'and-operands and-operands)
                (list 'or-operands or-operands)
                (list '= =)
                (list 'eq? eq?)
                )
  )
...
ev-or
  ...
  (save unev)
  (save env)
  (assign continue (label ev-or-continue))
  (goto (label eval-dispatch))
ev-or-continue
  (restore env)
  (restore unev)
  (test (op true?) (reg val))
  (branch (label ev-or-true))
  (test (op no-more-exps?) (reg unev))
  (branch (label ev-or-end))
  (assign exp (op first-exp) (reg unev))
  (assign unev (op rest-exps) (reg unev))
  (assign continue (label ev-or-continue))
  (save unev)
...
ev-and
  ...
  (save unev)
  (save env)
  (assign continue (label ev-and-continue))
  (goto (label eval-dispatch))
ev-and-continue
  (restore env)
  (restore unev)
  (test (op true?) (reg val))
  (branch (label ev-and-true))
  (assign val (reg val))
  (restore continue)
  (goto (reg continue))
ev-and-true
  (test (op no-more-exps?) (reg unev))
  (branch (label ev-and-end))
  (assign continue (label ev-and-continue))
  (assign exp (op first-exp) (reg unev))
  (assign unev (op rest-exps) (reg unev))
  (save unev)
