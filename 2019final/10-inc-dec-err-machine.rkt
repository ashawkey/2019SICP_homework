...
(define (make-machine register-names ops controller-text exception)
  (let ((machine (make-new-machine)))
    ...
    ((machine 'install-instruction-sequence)
     ;;; mod
     (assemble (install-exception controller-text exception) machine))
    machine))

;;; mod
(define (install-exception controller exception)
  (append (append controller
                  (append (list '(goto (label done)) 'error)
                          exception))
          (list 'done)))
...
(define (make-execution-procedure inst labels machine pc flag stack ops) 
  (cond ((eq? (car inst) 'assign)
        ...
        ((eq? (car inst) 'dec)
         (make-dec inst machine labels pc))
...
(define (make-inc inst machine labels pc)
  (let ([reg (get-register machine (cadr inst))])
    (lambda ()
      (let ([ctc (get-contents reg)])
        (if (number? ctc)
            (begin (set-contents! reg (+ ctc 1))
                   (advance-pc pc))
            (let ([insts (lookup-label labels 'error)])
              (set-contents! pc insts)))))))

(define (make-dec inst machine labels pc)
  (let ([reg (get-register machine (cadr inst))])
    (lambda ()
      (let ([ctc (get-contents reg)])
        (if (number? ctc)
...
(define (make-machine-from-text machine-text)
  ...
  (define exception (cadar (cddddr machine-text)))
  (make-machine reg-table op-table controller exception))
