  ...
  (if (null? clauses)
      ...
      (let ((first (car clauses))
        ...
        (cond ((cond-else-clause? first) (if (null? rest)
              ...
              ((cond-special-clause? first) (make-if (cond-predicate first)
...
(define (analyze-switch exp)
  (let ((target (cadr exp))
        (clauses (cddr exp)))
    (define (helper clauses env)      
      (let ((first (car clauses))
            (rest (cdr clauses)))
        (let ((current (car first))
              (actions (cdr first)))
          (if (or (eq? 'default current) (eq? ((analyze target) env) ((analyze current) env)))
              ((analyze-sequence actions) env)
              (helper rest env)))))
    (lambda (env) (helper clauses env))))
...
(define (analyze-while exp)
  (let ((test (cadr exp))
        (actions (cddr exp)))
    (define (helper env)
      (if (true? ((analyze test) env))
          (begin ((analyze-sequence actions) env) (helper env))
          (void)))
    (lambda (env)
      (helper env))))
...
(define (analyze-for exp)
  (let ((init (cadr exp))
        ...
        (actions (cddddr exp)))
    (define (helper env)
      (if (true? ((analyze pred) env))
          (if (run-for-sequence actions env)
              (begin ((analyze step) env) (helper env))
              (begin (set! break-flag 0) (void)))
          (void)))
    (lambda (env)
      ((analyze init) env)
      (helper env))))
...
(define (analyze-break)
  (lambda (env) (set! break-flag 1)))

(define (run-for-sequence exps env)
  (if (null? exps) true
      (let ((first (car exps))
            (rest (cdr exps)))
        ((analyze first) env)
