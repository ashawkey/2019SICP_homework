; http://lisp.test.openjudge.org/2019hw10/1/
#lang racket
  ...
  (cond ((primitive-procedure? procedure)
        ...
        ((compound-procedure? procedure)
         (eval-sequence (procedure-body procedure)
          (extend-environment  
           (mmap (lambda (x) (if (pair? x) (car x) x)) (procedure-parameters procedure))
           (list->mlist (list-of-different-values (procedure-parameters procedure) arguments env)) 
...
(define (list-of-different-values params exps env)
  (if (no-operands? exps)
      '()
      (let ((first-param (mcar params))
            (rest-param (mcdr params))
            (first (first-operand exps))
            (rest (rest-operands exps)))
        (if (pair? first-param)
            (let ((type (cadr first-param)))
              (if (eq? type 'lazy)
                  (cons (delay-it first env) (list-of-different-values rest-param rest env))
                  (cons (delay-memo-it first env) (list-of-different-values rest-param rest env))))
            (cons (eval first env) (list-of-different-values rest-param rest env))))))
